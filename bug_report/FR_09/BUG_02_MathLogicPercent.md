...
`discount amount = (total amount * coupon.discount_value) / 100`

If `coupon.discount_value` is `10` (representing 10%), the math evaluates to:
`500,000 * (1 - 10) = 500,000 * -9 = -4,500,000`
Then `final amount = total amount - discount amount` evaluates to:
`500,000 - (-4,500,000) = 5,000,000`.

**Fix:**
It should be:
`discount amount = (total amount * coupon.discount value) / 100`

## Screenshot

![Math Logic Percent Bug](../../bug_screenshot/FR_09/BUG_02.png)
